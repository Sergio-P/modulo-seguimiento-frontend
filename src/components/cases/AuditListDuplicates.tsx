...
export default function AuditListDuplicates() {
  ...
  const caseQuery = useQuery({
    ...
    queryFn: () => api.getComentariosByType("duplicate"),
  });
...
function AuditListTable({
  ...
  const columns = useMemo(
    () => [
      ...
      //   }
      // ),
      columnHelper.accessor("data.subcategoria", {
        header: "Subcategoría",
        size: 168,
      }),
      columnHelper.accessor("data.fecha_diagnostico", {
  ...
  return (
    <>
      <Datagrid
        table={table}
        title="Lista de Casos Duplicados"
        loading={loading}
      />
    </>
  );
